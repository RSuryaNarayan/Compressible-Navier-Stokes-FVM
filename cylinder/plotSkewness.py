...
skew = skew.reshape((nx,ny))

# Plot the cell positions with smoother contours
# Plot the grid lines for the mesh
plt.figure(figsize=(10, 6))
plt.contourf(x_coords, y_coords, skew, cmap='jet', levels=1000,extend='both')  # Increased levels for smoother contours
plt.colorbar(label='skewness')
plt.xlabel('x')
plt.ylabel('y')
plt.title(f'Cell skewness ({nx+1} x {ny+1})')
