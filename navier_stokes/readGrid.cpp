...
struct Cell {
    ...
    Eigen::Matrix<long double, 2, 1> gradU; //gradient vector for x-velocity (u)
    Eigen::Matrix<long double, 2, 1> gradV; //gradient vector for y-velocity (v)
    Eigen::Matrix<long double, 2, 1> gradT; //gradient vector for Temperature (T)
...
public:
    ...
    void generateCells() {
        ...
        dx = nodes[ny].x-nodes[0].x; //rough estimate
        ...
        for (int k=1; k<=nsteps; k++)
        {
            ...
            for (int iif=0; iif<nfaces; iif++)
            {
                ...
                //outlet flux (zero gradient)
                if (face.bc_index==2)
                {
                    #include "outletFlux.H"
                }
                ...
                // symmetry flux (same as slip-wall flux for inviscid)
                if (face.bc_index==4)
                {
                    #include "symmetryFlux.H"
                }
            }
            ...
            {
                //u-gradients
                long double gradU_x = 0.0;
                long double gradU_y = 0.0; 

                //v-gradients
                long double gradV_x = 0.0;
                long double gradV_y = 0.0; 

                //T-gradients
                long double gradT_x = 0.0;
                long double gradT_y = 0.0; 

                long double flip = -1.0;
                for (int iif=0; iif<4; iif++)
                {
                    //get face
                    Face& face = faces[cell.faces_index[iif]];

                    //if-else block to get primitive state at face
                    Eigen::Matrix<long double, 5, 1> V_f;
                    if (face.bc_index==0) // interior face
                    {
                        // get left and right states
                        Eigen::Matrix<long double, 5, 1> V_L  = cells[face.i_L].V;
                        Eigen::Matrix<long double, 5, 1> V_R  = cells[face.i_R].V;
                        //average onto the face
                        V_f  = (V_L + V_R)/2.0L;
                    }
                    else if (face.bc_index==1) // inlet
                    {
                        // get left and right states
                        Eigen::Matrix<long double, 5, 1> V_L; // this is just inlet condition
                        long double rho_in = P_inlet/R/T_inlet;
                        long double u_in = M_inlet * sqrt(GAMMA*R*T_inlet);
                        V_L(QRHO) = rho_in;
                        V_L(QU) = u_in;
                        V_L(QV) = 0.0;
                        V_L(QT) = T_inlet;
                    ...
                    else if (face.bc_index==2) // outlet
                    {
                        // get left and right states
                        Eigen::Matrix<long double, 5, 1> V_L  = cells[face.i_L].V; 
                        Eigen::Matrix<long double, 5, 1> V_R  = V_L; // zero gradient

                        //average onto the face for modified steger warming
                        V_f  = (V_L + V_R)/2.0L;
                    }
                    else if (face.bc_index==3) // wall
                    {
                        // get left and right states
                        ...
                        V_L(QU) = -1.0L*V_R(QU);
                        V_L(QV) = -1.0L*V_R(QV);
                        V_L(QT) = 2.0L*T_wall - V_R(QT);

                        //average onto the face for modified steger warming
                        V_f  = (V_L + V_R)/2.0L;
                    }
                    else if (face.bc_index==4) // symmetry (outlet)
                    {
                        // get left and right states
                        Eigen::Matrix<long double, 5, 1> V_L  = cells[face.i_L].V; 
                        Eigen::Matrix<long double, 5, 1> V_R  = V_L;
                    ...
                    //fill face values
                    long double u_face = V_f(QU);
                    long double v_face = V_f(QV);
                    long double T_face = V_f(QT);

                    //fill gradients
                    gradU_x += flip*u_face * face.n_x * face.S_f / cell.volume;
                    gradU_y += flip*u_face * face.n_y * face.S_f / cell.volume;

                    gradV_x += flip*v_face * face.n_x * face.S_f / cell.volume;
                    gradV_y += flip*v_face * face.n_y * face.S_f / cell.volume;

                    gradT_x += flip*T_face * face.n_x * face.S_f / cell.volume;
                    gradT_y += flip*T_face * face.n_y * face.S_f / cell.volume;

                    flip*=-1.0L;
                }

                cell.gradU<<gradU_x, gradU_y;
                cell.gradV<<gradV_x, gradV_y;
                cell.gradT<<gradT_x, gradT_y;
    ...
    void outputPrimitiveStateToCSV(const std::string& filename) const {
        ...
        if (file.is_open()) {
            file << "x_c,y_c,Ri,rho,u,v,p,T,gradUx,gradUy,gradVx,gradVy,gradTx,gradTy\n"; // CSV header for primitive state

            for (const auto& cell : cells) {
                file << cell.x_c << "," << cell.y_c << "," // Output cell centers for visualization
                     << cell.Ri(0)/cell.volume << "," << cell.V(0) << "," << cell.V(1) << "," << cell.V(2) << "," 
                     << cell.V(3) << "," << cell.V(4) << ","
                     << cell.gradU(0) << "," << cell.gradU(1) << "," //gradients
                     << cell.gradV(0) << "," << cell.gradV(1) << "," 
                     << cell.gradT(0) << "," << cell.gradT(1) << "\n"; // Output primitive state and gradients
            }
...
private:
    ...
    long double M_inlet = 1.5;
    long double T_inlet = 300; //K
    long double rho_inlet = 0.01; // kg/m3
    long double P_inlet = rho_inlet * R * T_inlet; //Pa
    long double T_wall = T_inlet;
    
    //simulation time
    int nsteps = 16000;
    long double dx;
    long double CFL=20.0;
    long double dt;
